...
def config_free_proxy(category):
    ...
    handle_proxy = str(input(
        bcolors.OKBLUE + '\nLet FG_Viewer handle proxies ? (recommended=No) [No/yes] : ' + bcolors.ENDC)).lower()
...
def create_config(config_path):
    ...
    print(bcolors.OKGREEN + '\n--> Your preferences are saved in config.json. You can always create a new config file from FG_Viewer.py' + bcolors.ENDC)
    print(bcolors.OKGREEN +
          '--> Or by running `python fg/config.py` ' + bcolors.ENDC)
